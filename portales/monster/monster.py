...
class Monster(Portal):
    ...
    def _buscar_keyword(self, keyword:str, n_pagina:int):
        ...
        driver = self._driver
        driver.get(f"{self._base_url}{ruta_busqueda}?{parametro_keyword}&{parametro_pagina}&geo=0&where=españa")
    ...
    def _get_title(self, position):
        ...
        return position.find_element(By.CSS_SELECTOR, 'a[data-test-id="svx-job-title"]').text.strip()

    def _get_companyname(self, position):
        try:
            empresa = position.find_element(By.CSS_SELECTOR, 'h3[data-test-id="svx-job-company"]').text
        except:
    ...
    def _get_location(self, position):
        return self._filtro.filtrar_localizacion(position.find_element(By.CSS_SELECTOR, 'p[data-test-id="svx-job-location"]').text.strip())

    def _get_skills(self, position):
        return self._filtro.filtrar_skills(position.text)

    def _get_publish_date(self,position):
        return self._filtro.filtrar_fecha(position.find_element(By.CSS_SELECTOR, 'span[data-test-id="svx-job-date"]').text)
